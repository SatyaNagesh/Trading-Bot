"""holdout_3 acquisition — untouched confirmation universe for Composite V3.

Reuses the frozen ingestion/audit code verbatim; only the data store root is
injected (data/holdout_3). Symbols come exclusively from the pre-registered
data/holdout_3/universe.json (written by select_holdout3_universe.py BEFORE
any download). Writes manifest, quarantine, audit, eligible_symbols,
splits.json and dataset_lock.json.

Usage:
  python -m packages.research.acquire_holdout3             # fetch + audit
  python -m packages.research.acquire_holdout3 --verify    # checksum audit only
"""
...
def _wire() -> tuple:
    ...
    di.store_root = lambda: HOLD3
    da.store_root = di.store_root
    da.QUARANTINE_LOG = str(HOLD3 / "quarantine.json")
    return di, da


def run_acquire(di, start: str, end: str, sleep: float) -> None:
    univ = json.loads((HOLD3 / "universe.json").read_text())
...
def lock_dataset(di) -> None:
    man_bytes = (HOLD3 / "manifest.json").read_bytes()
    sha = hashlib.sha256(man_bytes).hexdigest()
    kept = json.loads((HOLD3 / "eligible_symbols.json").read_text())["kept"]
    (HOLD3 / "dataset_lock.json").write_text(json.dumps({
        "sha256": sha,
        "n_symbols": len(kept),
        "bars_total": int(sum(e["bars"] for e in di.load_manifest())),
        "universe_ref": "data/holdout_3/universe.json (BEFORE any download)",
        "locked_utc": datetime.now(timezone.utc).isoformat(),
        "note": "holdout_3 confirmation universe for Composite V3; ANY evaluation "
                "must use exactly this corpus"
    }, indent=2))
...
def main() -> None:
    ...
    ap.add_argument("--finish", action="store_true", help="skip download; run audit/eligible/lock from manifest")
    args = ap.parse_args()

    di, da = _wire()
    HOLD3.mkdir(parents=True, exist_ok=True)
    ...
    log = da.cleanse()
    (HOLD3 / "quarantine.json").write_text(json.dumps(log, indent=2, default=str))
    purged = purge_empty(di, log)
    audit = da.run_audit()
    (HOLD3 / "audit.json").write_text(json.dumps(audit, indent=2, default=str))
    ...
    (HOLD3 / "eligible_symbols.json").write_text(
        json.dumps({"kept": sorted(kept), "dropped": dropped}, indent=2))
    print(f"eligible (bars>=700): {len(kept)}  dropped: {len(dropped)}")

    # verbatim frozen split boundaries (same as DEVELOPMENT / HOLDOUT / HOLDOUT_2)
    (HOLD3 / "splits.json").write_text(json.dumps({
        "train": {"start": "2014-01-01", "end": "2023-01-01"},
        "validation": {"start": "2023-01-01", "end": "2025-07-01"},
        "final_oos": {"start": "2025-07-01", "end": "2026-09-06"},
    }, indent=2))
